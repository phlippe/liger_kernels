...
@pytest.mark.parametrize(
    "batch_size,context_length,vocab_size,embed_dim",
    [(1, 4, 32, 16), (4, 2048, 50304, 1024)],
)
...
def test_liger_fused_linear_cross_entropy(
    ...
    use_bias: bool,
    dtype: jnp.dtype,
    add_trivial_operations: bool,
):
    """Test the liger fused linear cross entropy kernel."""
    # Generate random inputs
    input_tensor = jax.random.normal(
        jax.random.PRNGKey(0), (batch_size, context_length, embed_dim)
    ).astype(dtype) / np.sqrt(vocab_size)
    target_tensor = jax.random.randint(
        jax.random.PRNGKey(1), (batch_size, context_length), 0, vocab_size
    )
    target_mask = (
        jax.random.bernoulli(jax.random.PRNGKey(2), 0.5, (batch_size, context_length))
        == 1
    )
    params = {
        "weight": jax.random.normal(jax.random.PRNGKey(3), (embed_dim, vocab_size))
    }
    if use_bias:
    ...
    jax_loss, (jax_x_grads, jax_param_grads) = jax_grad_fn(
        input_tensor, params, target_tensor, target_mask
    )
    jax_arrays = {
        "loss": jax_loss,
        "x": jax_x_grads,
        "weight": jax_param_grads["weight"],
    }
    if use_bias:
    ...
    (
        liger_arrays["loss"],
        (liger_arrays["x"], liger_arrays["weight"], liger_arrays["bias"]),
    ) = liger_grad_fn(
        input_tensor,
        params["weight"],
        target_tensor,
    ...
    for key in jax_arrays:
        if jax_arrays[key] is not None:
            assert (
                liger_arrays[key].shape == jax_arrays[key].shape
            ), f"The shape of {key} does not match."
            assert (
                liger_arrays[key].dtype == jax_arrays[key].dtype
            ), f"The dtype of {key} does not match."
            assert jnp.all(
                jnp.isfinite(jax_arrays[key])
            ), f"JAX {key} contains non-finite values: {jax_arrays[key]}."
            assert jnp.all(
