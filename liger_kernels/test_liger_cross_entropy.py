...
@pytest.mark.parametrize("batch_size,context_length,vocab_size", [(3, 4, 8), (4, 128, 50304)])
@pytest.mark.parametrize("mask_prob", [0.9, 0.5])
def test_liger_cross_entropy(batch_size: int, context_length: int, vocab_size: int, mask_prob: float):
    """Test the liger cross entropy kernel."""
    # Generate random inputs
    input_tensor = jax.random.normal(jax.random.PRNGKey(0), (batch_size, context_length, vocab_size))
    target_tensor = jax.random.randint(jax.random.PRNGKey(0), (batch_size, context_length), 0, vocab_size)
    target_mask = jax.random.bernoulli(jax.random.PRNGKey(0), mask_prob, (batch_size, context_length)) == 1
    ...
    liger_grad_fn = jax.value_and_grad(partial(liger_cross_entropy, reduction="mean", last_layer=True))
    ...
    np.testing.assert_allclose(jax_loss, liger_loss, rtol=1e-5, atol=1e-5, err_msg="Losses do not match.")
    np.testing.assert_allclose(jax_grads, liger_grads, rtol=1e-5, atol=1e-5, err_msg="Gradients do not match.")
